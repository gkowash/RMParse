...
class CommandCase(Enum):
    ...
    STREET_FLOW = auto()
    STREET_INLET = auto()
    SUBAREA_ADDITION = auto()
    PIPEFLOW_PROGRAM = auto()
    PIPEFLOW_USER = auto()
    CHANNEL_IMPROVED = auto()
    CHANNEL_IRREGULAR = auto()
    USER = auto()
    CONFLUENCE_MINOR = auto()
    CONFLUENCE_MAIN = auto()
...
COMMAND_MAP = {
    ...
    CommandCase.STREET_INLET: "(command flag undefined)", #flag unavailable at time of writing; set if required
...
def parse_data_from(lines: List[str]) -> List[Tuple[str, float, float]]:
